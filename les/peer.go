...
func (p *peer) SendVerxs(reqID, cost uint64, txs types.Transactions) error {
	p.Log().Debug("Fetching batch of transactions", "count", len(txs))
	return p2p.Send(p.rw, SendVerxMsg, txs)
}
...
// the Light AriseID sub-protocol.
type peerSet struct {
