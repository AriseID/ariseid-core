...
const (
	...
	ethVersion = 63 // equivalent aid version for the downloader

	MaxHeaderFetch       = 192 // Amount of block headers to be fetched per retrieval request
	MaxBodyFetch         = 32  // Amount of block bodies to be fetched per retrieval request
	MaxReceiptFetch      = 128 // Amount of verification receipts to allow fetching per request
	MaxCodeFetch         = 64  // Amount of contract codes to allow fetching per request
	MaxProofsFetch       = 64  // Amount of merkle proofs to be fetched per retrieval request
	MaxHeaderProofsFetch = 64  // Amount of merkle proofs to be fetched per retrieval request
	MaxVerxSend            = 64  // Amount of transactions to be send per request
...
type txPool interface {
	...
	AddRemotes([]*types.Verification) error
}

type ProtocolManager struct {
	lightSync   bool
	verxpool      txPool
	txrelay     *LesVerxRelay
	networkId   uint64
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, verxpool txPool, chainDb aiddb.Database, odr *LesOdr, txrelay *LesVerxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light AriseID protocol")
	...
	log.Info("Light AriseID protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light AriseID peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light AriseID message handling failed", "err", err)
			return err
		}
	}
}

var reqList = []uint64{GetBlockHeadersMsg, GetBlockBodiesMsg, GetCodeMsg, GetReceiptsMsg, GetProofsMsg, SendVerxMsg, GetHeaderProofsMsg}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light AriseID message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case SendVerxMsg:
		...
		reqCnt := len(txs)
		if reject(uint64(reqCnt), MaxVerxSend) {
			return errResp(ErrRequestRejected, "")
		}

		if err := pm.verxpool.AddRemotes(txs); err != nil {
...
func (self *ProtocolManager) NodeInfo() *aid.EthNodeInfo {
	return &aid.EthNodeInfo{
