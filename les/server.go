...
func NewLesServer(aid *aid.AriseID, config *aid.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(aid.BlockChain().Config(), false, config.NetworkId, aid.EventMux(), aid.Engine(), newPeerSet(), aid.BlockChain(), aid.VerxPool(), aid.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	pm.blockLoop()
	...
	srv := &LesServer{
		protocolManager: pm,
		quitSync:        quitSync,
		lesTopic:        lesTopic(aid.BlockChain().Genesis().Hash()),
	}
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(aid.ChainDb())
	return srv, nil
}
...
func newCostStats(db aiddb.Database) *requestCostStats {
...
func getChtRoot(db aiddb.Database, num uint64) common.Hash {
...
func storeChtRoot(db aiddb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db aiddb.Database) bool {
