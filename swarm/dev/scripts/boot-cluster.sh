...
#
# The cluster contains a bootnode, a idd node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the idd and
# swarm nodes
start_bootnode() {
...
# start_idd_node starts a idd node with --datadir pointing at <base-dir>/idd
# and a single, unlocked account with password "idd"
start_idd_node() {
  local dir="${base_dir}/idd"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get idd account address"
  fi
  ...
  start_node "idd" "${GETH_IP}" "$(which idd)" ${args[@]}
}
...
create_account() {
  ...
  idd --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
