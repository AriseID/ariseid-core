...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("aid", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("aid", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientReconnect(t *testing.T) {
	...
	// We spawn multiple calls here to check whid this hangs somehow.
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("ariseid-core-test-ipc-%d-%d", os.Getpid(), rand.Int63())
