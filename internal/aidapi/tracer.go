...
func (jst *JavascriptTracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, life, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
...
func (jst *JavascriptTracer) CaptureEnd(output []byte, lifeUsed uint64, t time.Duration, err error) error {
