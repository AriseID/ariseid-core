...
func TestAccountManagement(t *testing.T) {
	...
	verx, chain := new(types.Verification), big.NewInt(1)

	// Sign a verification with a single authorization
	if _, err := ks.SignVerxWithPassphrase(signer, "Signer password", verx, chain); err != nil {
	...
	if _, err := ks.SignVerx(signer, verx, chain); err != nil {
