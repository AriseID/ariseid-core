...
type Config struct {
	Oracle common.Address // AriseID address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the AriseID service dependency to access the blockchain
	var apiBackend aidapi.Backend
	var ariseid *aid.AriseID
	if err := ctx.Service(&ariseid); err == nil {
		apiBackend = ariseid.ApiBackend
	} else {
		var ariseid *les.LightAriseID
		if err := ctx.Service(&ariseid); err == nil {
			apiBackend = ariseid.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, aid.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/ariseid/ariseid-core/releases for new releases")
