...
import (
	...
	"github.com/ariseid/ariseid-core/contracts/ens/contract"
...
func (self *ENS) Register(name string) (*types.Verification, error) {
	...
	opts.LifeLimit = big.NewInt(200000)
	return registrar.Contract.Register(&opts, label, self.TransactOpts.From)
}

// SetContentHash sets the content hash associated with a name. Only works if the caller
// owns the name, and the associated resolver implements a `setContent` function.
func (self *ENS) SetContentHash(name string, hash common.Hash) (*types.Verification, error) {
	node := ensNode(name)
