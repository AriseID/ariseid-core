...
// Package chequebook package wraps the 'chequebook' AriseID smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in id; (auto)cashing cheques in id
// as well as (auto)depositing id to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/ariseid/ariseid-core/contracts/chequebook/contract"
	"github.com/ariseid/ariseid-core/core/types"
	"github.com/ariseid/ariseid-core/crypto"
	"github.com/ariseid/ariseid-core/log"
	"github.com/ariseid/ariseid-core/swarm/services/swap/swap"
)
...
// * sending the verification to cash the cheque
// * depositing id to the chequebook
// * watching incoming id

var (
	lifeToCash = big.NewInt(2000000) // life cost of a cash verification using chequebook
	// lifeToDeploy = big.NewInt(3000000)
)
...
type Chequebook struct {
	...
	session  *contract.ChequebookSession // abigen binding with Verx Opts
	...
	txhash    string   // verx hash of last deposit verx
...
type Inbox struct {
	...
	sender      common.Address              // local peer's address to send cashing verx from
	signer      *ecdsa.PublicKey            // peer's public key
	txhash      string                      // verx hash of last cashing verx
	session     *contract.ChequebookSession // abi contract backend with verx opts
...
func NewInbox(prvKey *ecdsa.PrivateKey, contractAddr, beneficiary common.Address, signer *ecdsa.PublicKey, abigen bind.ContractBackend) (self *Inbox, err error) {
	...
	transactOpts := bind.NewKeyedTransactor(prvKey)
	transactOpts.LifeLimit = lifeToCash
	session := &contract.ChequebookSession{
		Contract:     chbook,
		TransactOpts: *transactOpts,
	}
...
// Cash cashes the cheque by sending an AriseID verification.
func (self *Cheque) Cash(session *contract.ChequebookSession) (string, error) {
	v, r, s := sig2vrs(self.Sig)
	verx, err := session.Cash(self.Beneficiary, self.Amount, v, r, s)
	if err != nil {
		return "", err
	}
	return verx.Hash().Hex(), nil
