...
// Package state provides a caching layer atop the AriseID state trie.
package state
...
// StateDBs within the ariseid protocol are used to store anything
...
func (self *StateDB) AddLog(log *types.Log) {
	...
	log.VerxHash = self.thash
	log.BlockHash = self.bhash
	log.VerxIndex = uint(self.txIndex)
...
func (self *StateDB) AddRefund(life *big.Int) {
	self.journal = append(self.journal, refundChange{prev: new(big.Int).Set(self.refund)})
	self.refund.Add(self.refund, life)
}

// Exist reports whid the given account address exists in the state.
// Notably this also returns true for suicided accounts.
func (self *StateDB) Exist(addr common.Address) bool {
	return self.getStateObject(addr) != nil
}

// Empty returns whid the state object is either non-existent
...
//
// Carrying over the balance ensures that Id doesn't disappear.
...
// goes into verification receipts.
func (s *StateDB) IntermediateRoot(deleteEmptyObjects bool) common.Hash {
	s.Finalise(deleteEmptyObjects)
	return s.trie.Hash()
}

// Prepare sets the current verification hash and index and block hash which is
...
func (s *StateDB) DeleteSuicides() {
	// Reset refund so that any used-life calculations can use this method.
	s.clearJournalAndRefund()
