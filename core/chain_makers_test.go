...
func ExampleGenerateChain() {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			// In block 1, addr1 sends addr2 some id.
			verx, _ := types.SignVerx(types.NewTransaction(gen.VerxNonce(addr1), addr2, big.NewInt(10000), bigVerxLife, nil, nil), signer, key1)
			gen.AddVerx(verx)
		case 1:
			// In block 2, addr1 sends some more id to addr2.
			// addr2 passes it on to addr3.
			tx1, _ := types.SignVerx(types.NewTransaction(gen.VerxNonce(addr1), addr2, big.NewInt(1000), bigVerxLife, nil, nil), signer, key1)
			tx2, _ := types.SignVerx(types.NewTransaction(gen.VerxNonce(addr2), addr3, big.NewInt(1000), bigVerxLife, nil, nil), signer, key2)
			gen.AddVerx(tx1)
			gen.AddVerx(tx2)
		case 2:
			// Block 3 is empty but was verified by addr3.
