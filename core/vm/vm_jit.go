...
// More: https://github.com/ariseid/evmjit
#cgo LDFLAGS: -levmjit
*/
import "C"
...
func (self *JitVm) Run(me, caller ContextRef, code []byte, value, life, price *big.Int, callData []byte) (ret []byte, err error) {
	...
	if Precompiled[string(me.Address())] != nil {
		...
		stdVm := New(self.env)
		return stdVm.Run(me, caller, code, value, life, price, callData)
	}
	...
	self.data.life = life.Int64()
	self.data.lifePrice = price.Int64()
	...
	self.data.lifeLimit = big2llvm(self.env.LifeLimit())
	...
	if retCode < 0 {
		...
		life.SetInt64(0) // Set life to 0, JIT does not bother
	} else {
		life.SetInt64(self.data.life)
		if retCode == 1 { // RETURN
...
func env_sstore(vmPtr unsafe.Pointer, indexPtr unsafe.Pointer, valuePtr unsafe.Pointer) {
	...
	if len(value) == 0 {
		...
		if len(prevValue) != 0 {
			vm.Env().State().Refund(vm.callerAddr, LifeSStoreRefund)
		}
	}
...
//export env_call
func env_call(_vm unsafe.Pointer, _life *int64, _receiveAddr unsafe.Pointer, _value unsafe.Pointer, inDataPtr unsafe.Pointer, inDataLen uint64, outDataPtr *byte, outDataLen uint64, _codeAddr unsafe.Pointer) bool {
	vm := (*JitVm)(_vm)
	...
	if balance.Cmp(value) >= 0 {
		...
		life := big.NewInt(*_life)
		var out []byte
		var err error
		if bytes.Equal(codeAddr, receiveAddr) {
			out, err = vm.env.Call(vm.me, codeAddr, inData, life, vm.price, value)
		} else {
			out, err = vm.env.CallCode(vm.me, codeAddr, inData, life, vm.price, value)
		}
		*_life = life.Int64()
		if err == nil {
			copy(outData, out)
...
//export env_create
func env_create(_vm unsafe.Pointer, _life *int64, _value unsafe.Pointer, initDataPtr unsafe.Pointer, initDataLen uint64, _result unsafe.Pointer) {
	vm := (*JitVm)(_vm)
	...
	*result = i256{}
	...
	life := big.NewInt(*_life)
	ret, suberr, ref := vm.env.Create(vm.me, nil, initData, life, vm.price, value)
	if suberr == nil {
		dataLife := big.NewInt(int64(len(ret))) // TODO: Not the best design. env.Create can do it, it has the reference to life counter
		dataLife.Mul(dataLife, params.CreateDataLife)
		life.Sub(life, dataLife)
