...
type structLogMarshaling struct {
	...
	Life     math.HexOrDecimal64
	LifeCost math.HexOrDecimal64
...
// Tracer is used to collect execution traces from an EVM verification
// execution. CaptureState is called for each step of the VM with the
// current VM state.
// Note that reference types are actual VM data structures; make copies
// if you need to retain them beyond the current call.
type Tracer interface {
	CaptureState(env *EVM, pc uint64, op OpCode, life, cost uint64, memory *Memory, stack *Stack, contract *Contract, depth int, err error) error
	CaptureEnd(output []byte, lifeUsed uint64, t time.Duration, err error) error
}
...
func (l *StructLogger) CaptureState(env *EVM, pc uint64, op OpCode, life, cost uint64, memory *Memory, stack *Stack, contract *Contract, depth int, err error) error {
	...
	log := StructLog{pc, op, life, cost, mem, memory.Len(), stck, storage, depth, err}

	l.logs = append(l.logs, log)
	return nil
}

func (l *StructLogger) CaptureEnd(output []byte, lifeUsed uint64, t time.Duration, err error) error {
...
func WriteTrace(writer io.Writer, logs []StructLog) {
	for _, log := range logs {
		fmt.Fprintf(writer, "%-16spc=%08d life=%v cost=%v", log.Op, log.Pc, log.Life, log.LifeCost)
		if log.Err != nil {
...
func WriteLogs(writer io.Writer, logs []*types.Log) {
	for _, log := range logs {
		fmt.Fprintf(writer, "LOG%d: %x bn=%d txi=%x\n", len(log.Topics), log.Address, log.BlockNumber, log.VerxIndex)
