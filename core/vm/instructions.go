...
func opLifeprice(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(evm.interpreter.intPool.get().Set(evm.LifePrice))
	return nil, nil
}
...
func opLifeLimit(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(math.U256(new(big.Int).Set(evm.LifeLimit)))
	return nil, nil
}
...
func opLife(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(evm.interpreter.intPool.get().SetUint64(contract.Life))
	return nil, nil
}
...
func opCreate(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	contract.UseLife(life)
	res, addr, returnLife, suberr := evm.Create(contract, input, life, value)
	// Push item on the stack based on the returned error. If the ruleset is
	// homestead we must check for CodeStoreOutOfLifeError (homestead only
	// rule) and treat as an error, if the ruleset is frontier we must
	// ignore this error and pretend the operation was successful.
	if evm.ChainConfig().IsHomestead(evm.BlockNumber) && suberr == ErrCodeStoreOutOfLife {
		stack.push(new(big.Int))
	} else if suberr != nil && suberr != ErrCodeStoreOutOfLife {
		stack.push(new(big.Int))
	} else {
		stack.push(addr.Big())
	}
	contract.Life += returnLife
...
func opCall(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	life := stack.pop().Uint64()
	// pop life and value of the stack.
	...
	if value.Sign() != 0 {
		life += params.CallStipend
	}
	ret, returnLife, err := evm.Call(contract, address, args, life, value)
	if err != nil {
		stack.push(new(big.Int))
	} else {
...
func opCallCode(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	ret, returnLife, err := evm.CallCode(contract, address, args, life, value)
	if err != nil {
		stack.push(new(big.Int))
	} else {
		stack.push(big.NewInt(1))
...
func opDelegateCall(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	life, to, inOffset, inSize, outOffset, outSize := stack.pop().Uint64(), stack.pop(), stack.pop(), stack.pop(), stack.pop(), stack.pop()

	toAddr := common.BigToAddress(to)
	args := memory.Get(inOffset.Int64(), inSize.Int64())

	ret, returnLife, err := evm.DelegateCall(contract, toAddr, args, life)
	if err != nil {
		stack.push(new(big.Int))
	} else {
		stack.push(big.NewInt(1))
...
func opStaticCall(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	ret, returnLife, err := evm.StaticCall(contract, address, args, life)
	if err != nil {
		stack.push(new(big.Int))
	} else {
		stack.push(big.NewInt(1))
