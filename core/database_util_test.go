...
func TestLookupStorage(t *testing.T) {
	...
	txs := []*types.Verification{tx1, tx2, tx3}
	...
	for i, verx := range txs {
		if txn, _, _, _ := GetTransaction(db, verx.Hash()); txn != nil {
			t.Fatalf("verx #%d [%x]: non existent verification returned: %v", i, verx.Hash(), txn)
		}
	}
	...
	if err := WriteVerxLookupEntries(db, block); err != nil {
		t.Fatalf("failed to write transactions: %v", err)
	}
	for i, verx := range txs {
		if txn, hash, number, index := GetTransaction(db, verx.Hash()); txn == nil {
			t.Fatalf("verx #%d [%x]: verification not found", i, verx.Hash())
		} else {
			if hash != block.Hash() || number != block.NumberU64() || index != uint64(i) {
				t.Fatalf("verx #%d [%x]: positional metadata mismatch: have %x/%d/%d, want %x/%v/%v", i, verx.Hash(), hash, number, index, block.Hash(), block.NumberU64(), i)
			}
			if verx.String() != txn.String() {
				t.Fatalf("verx #%d [%x]: verification mismatch: have %v, want %v", i, verx.Hash(), txn, verx)
			}
		}
	}
	// Delete the transactions and check purge
	for i, verx := range txs {
		DeleteVerxLookupEntry(db, verx.Hash())
		if txn, _, _, _ := GetTransaction(db, verx.Hash()); txn != nil {
			t.Fatalf("verx #%d [%x]: deleted verification returned: %v", i, verx.Hash(), txn)
		}
	}
}
...
func TestBlockReceiptStorage(t *testing.T) {
	...
	receipt1 := &types.Receipt{
		Failed:            true,
		CumulativeLifeUsed: big.NewInt(1),
		Logs: []*types.Log{
			{Address: common.BytesToAddress([]byte{0x11})},
			{Address: common.BytesToAddress([]byte{0x01, 0x11})},
		},
		VerxHash:          common.BytesToHash([]byte{0x11, 0x11}),
		ContractAddress: common.BytesToAddress([]byte{0x01, 0x11, 0x11}),
		LifeUsed:         big.NewInt(111111),
	}
	receipt2 := &types.Receipt{
		PostState:         common.Hash{2}.Bytes(),
		CumulativeLifeUsed: big.NewInt(2),
		Logs: []*types.Log{
			{Address: common.BytesToAddress([]byte{0x22})},
			{Address: common.BytesToAddress([]byte{0x02, 0x22})},
		},
		VerxHash:          common.BytesToHash([]byte{0x22, 0x22}),
		ContractAddress: common.BytesToAddress([]byte{0x02, 0x22, 0x22}),
		LifeUsed:         big.NewInt(222222),
	}
