...
func TestEIP155Signing(t *testing.T) {
	...
	verx, err := SignVerx(NewTransaction(0, addr, new(big.Int), new(big.Int), new(big.Int), nil), signer, key)
	if err != nil {
		t.Fatal(err)
	}

	from, err := Sender(signer, verx)
	if err != nil {
		t.Fatal(err)
	}
	if from != addr {
...
func TestEIP155ChainId(t *testing.T) {
	...
	if !verx.Protected() {
		t.Fatal("expected verx to be protected")
	}

	if verx.ChainId().Cmp(signer.chainId) != 0 {
		t.Error("expected chainId to be", signer.chainId, "got", verx.ChainId())
	}

	verx = NewTransaction(0, addr, new(big.Int), new(big.Int), new(big.Int), nil)
	verx, err = SignVerx(verx, HomesteadSigner{}, key)
	if err != nil {
		t.Fatal(err)
	}

	if verx.Protected() {
		t.Error("didn't expect verx to be protected")
	}

	if verx.ChainId().Sign() != 0 {
		t.Error("expected chain id to be 0 got", verx.ChainId())
	}
}
...
func TestEIP155SigningVitalik(t *testing.T) {
	...
	for i, test := range []struct {
		...
		err := rlp.DecodeBytes(common.Hex2Bytes(test.txRlp), &verx)
		if err != nil {
			t.Errorf("%d: %v", i, err)
			continue
		}
...
func TestChainId(t *testing.T) {
	...
	verx := NewTransaction(0, common.Address{}, new(big.Int), new(big.Int), new(big.Int), nil)

	var err error
	verx, err = SignVerx(verx, NewEIP155Signer(big.NewInt(1)), key)
	if err != nil {
		t.Fatal(err)
	}

	_, err = Sender(NewEIP155Signer(big.NewInt(2)), verx)
	if err != ErrInvalidChainId {
		t.Error("expected error:", ErrInvalidChainId)
	}

	_, err = Sender(NewEIP155Signer(big.NewInt(1)), verx)
	if err != nil {
