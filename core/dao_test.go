...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, &proConf, idhash.NewFaker(), vm.Config{})
	defer proBc.Stop()

	conDb, _ := aiddb.NewMemDatabase()
	...
	conBc, _ := NewBlockChain(conDb, &conConf, idhash.NewFaker(), vm.Config{})
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db, _ = aiddb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, &conConf, idhash.NewFaker(), vm.Config{})
		defer bc.Stop()
		...
		db, _ = aiddb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, &proConf, idhash.NewFaker(), vm.Config{})
		defer bc.Stop()
