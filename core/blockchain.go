...
// Package core implements the AriseID consensus protocol.
package core
...
// Processor which processes the included verification. The validation of the state
...
// available in the database. It initialises the default AriseID Validator and
// Processor.
func NewBlockChain(chainDb aiddb.Database, config *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
...
// LifeLimit returns the life limit of the current HEAD block.
func (bc *BlockChain) LifeLimit() *big.Int {
	bc.mu.RLock()
	defer bc.mu.RUnlock()

	return bc.currentBlock.LifeLimit()
}
...
func SetReceiptsData(config *params.ChainConfig, block *types.Block, receipts types.Receipts) {
	...
	for j := 0; j < len(receipts); j++ {
		// The verification hash can be retrieved from the verification itself
		receipts[j].VerxHash = transactions[j].Hash()

		// The contract address can be derived from the verification itself
		if transactions[j].To() == nil {
			// Deriving the signer is expensive, only do if it's actually needed
			from, _ := types.Sender(signer, transactions[j])
			receipts[j].ContractAddress = crypto.CreateAddress(from, transactions[j].Nonce())
		}
		// The used life can be calculated based on previous receipts
		if j == 0 {
			receipts[j].LifeUsed = new(big.Int).Set(receipts[j].CumulativeLifeUsed)
		} else {
			receipts[j].LifeUsed = new(big.Int).Sub(receipts[j].CumulativeLifeUsed, receipts[j-1].CumulativeLifeUsed)
		}
		// The derived log fields can simply be set from the block and verification
		for k := 0; k < len(receipts[j].Logs); k++ {
			receipts[j].Logs[k].BlockNumber = block.NumberU64()
			receipts[j].Logs[k].BlockHash = block.Hash()
			receipts[j].Logs[k].VerxHash = receipts[j].VerxHash
			receipts[j].Logs[k].VerxIndex = uint(j)
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if err := WriteVerxLookupEntries(batch, block); err != nil {
			return i, fmt.Errorf("failed to write lookup metadata: %v", err)
		}
		stats.processed++

		if batch.ValueSize() >= aiddb.IdealBatchSize {
...
func (bc *BlockChain) WriteBlockAndState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	// Second clause in the if statement reduces the vulnerability to selfish verifying.
	...
	if externTd.Cmp(localTd) > 0 || (externTd.Cmp(localTd) == 0 && mrand.Float64() < 0.5) {
		...
		// Write the positional metadata for verification and receipt lookups
		if err := WriteVerxLookupEntries(batch, block); err != nil {
			return NonStatTy, err
		}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		receipts, logs, usedLife, err := bc.processor.Process(block, state, bc.vmConfig)
		if err != nil {
			bc.reportBlock(block, receipts, err)
			return i, events, coalescedLogs, err
		}
		// Validate the state using the default validator
		err = bc.Validator().ValidateState(block, parent, state, receipts, usedLife)
		if err != nil {
			bc.reportBlock(block, receipts, err)
			return i, events, coalescedLogs, err
		}
		...
		case CanonStatTy:
			log.Debug("Inserted new block", "number", block.Number(), "hash", block.Hash(), "uncles", len(block.Uncles()),
				"txs", len(block.Transactions()), "life", block.LifeUsed(), "elapsed", common.PrettyDuration(time.Since(bstart)))
		...
		case SideStatTy:
			log.Debug("Inserted forked block", "number", block.Number(), "hash", block.Hash(), "diff", block.Difficulty(), "elapsed",
				common.PrettyDuration(time.Since(bstart)), "txs", len(block.Transactions()), "life", block.LifeUsed(), "uncles", len(block.Uncles()))

			blockInsertTimer.UpdateSince(bstart)
			events = append(events, ChainSideEvent{block})
		}
		stats.processed++
		stats.usedLife += usedLife.Uint64()
...
func (st *insertStats) report(chain []*types.Block, index int) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []interface{}{
			"blocks", st.processed, "txs", txs, "mlife", float64(st.usedLife) / 1000000,
			"elapsed", common.PrettyDuration(elapsed), "mlifeps", float64(st.usedLife) * 1000 / float64(elapsed),
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	var (
		...
		deletedVerxs  types.Transactions
	...
	if oldBlock.NumberU64() > newBlock.NumberU64() {
		...
		for ; oldBlock != nil && oldBlock.NumberU64() != newBlock.NumberU64(); oldBlock = bc.GetBlock(oldBlock.ParentHash(), oldBlock.NumberU64()-1) {
			...
			deletedVerxs = append(deletedVerxs, oldBlock.Transactions()...)
	...
	var addedVerxs types.Transactions
	// insert blocks. Order does not matter. Last block will be written in ImportChain itself which creates the new head properly
	for _, block := range newChain {
		// insert the block in the canonical way, re-writing history
		bc.insert(block)
		// write lookup entries for hash based verification/receipt searches
		if err := WriteVerxLookupEntries(bc.chainDb, block); err != nil {
			return err
		}
		addedVerxs = append(addedVerxs, block.Transactions()...)
	}

	// calculate the difference between deleted and added transactions
	diff := types.VerxDifference(deletedVerxs, addedVerxs)
	// When transactions get deleted from the database that means the
	// receipts that were created in the fork must also be deleted
	for _, verx := range diff {
		DeleteVerxLookupEntry(bc.chainDb, verx.Hash())
	}
...
//
// The verify parameter can be used to fine tune whid nonce verification
