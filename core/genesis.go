...
type Genesis struct {
	...
	LifeLimit   uint64              `json:"lifeLimit"   gencodec:"required"`
...
func SetupGenesisBlock(db aiddb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	// Check whid the genesis block is already written.
	if genesis != nil {
...
func (g *Genesis) ToBlock() (*types.Block, *state.StateDB) {
	...
	head := &types.Header{
		...
		Extra:      g.ExtraData,
		LifeLimit:   new(big.Int).SetUint64(g.LifeLimit),
		LifeUsed:    new(big.Int).SetUint64(g.LifeUsed),
	...
	if g.LifeLimit == 0 {
		head.LifeLimit = params.GenesisLifeLimit
	}
	if g.Difficulty == nil {
...
func (g *Genesis) Commit(db aiddb.Database) (*types.Block, error) {
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db aiddb.Database) *types.Block {
...
func GenesisBlockForTesting(db aiddb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the AriseID main net genesis block.
...
// DevGenesisBlock returns the 'idd --dev' genesis block.
