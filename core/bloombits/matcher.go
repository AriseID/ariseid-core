...
type Matcher struct {
	...
	running uint32 // Atomic flag whid a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors toidder.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them toidder
				var orVector []byte
...
// the reuested retrieval queue to be serviced toidder with other sessions.
//
