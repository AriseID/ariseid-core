...
creating a collection of devp2p nodes, connecting them toidder to form a
...
* `Expect` - an expectation function which returns whid or not a
...
As a concrete example, consider a simulated network of AriseID nodes. An
`Action` could be the sending of a verification, `Expect` it being included in
a block, and `Trigger` a check for every block that is verified.
