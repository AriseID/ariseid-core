...
// The nodes were obtained by running testnet.verify with a random NodeID as target.
var lookupTestnet = &preverifiedTestnet{
...
type preverifiedTestnet struct {
	target    NodeID
...
func (tn *preverifiedTestnet) sendFindnode(to *Node, target NodeID) {
	panic("sendFindnode called")
}

func (tn *preverifiedTestnet) sendFindnodeHash(to *Node, target common.Hash) {
...
func (tn *preverifiedTestnet) sendPing(to *Node, addr *net.UDPAddr, topics []Topic) []byte {
	injectResponse(tn.net, to, pongPacket, &pong{ReplyTok: []byte{1}})
	return []byte{1}
}

func (tn *preverifiedTestnet) send(to *Node, ptype nodeEvent, data interface{}) (hash []byte) {
	switch ptype {
	case pingPacket:
		injectResponse(tn.net, to, pongPacket, &pong{ReplyTok: []byte{1}})
	case pongPacket:
...
func (tn *preverifiedTestnet) sendNeighbours(to *Node, nodes []*Node) {
	panic("sendNeighbours called")
}

func (tn *preverifiedTestnet) sendTopicQuery(to *Node, topic Topic) {
	panic("sendTopicQuery called")
}

func (tn *preverifiedTestnet) sendTopicNodes(to *Node, queryHash common.Hash, nodes []*Node) {
	panic("sendTopicNodes called")
}

func (tn *preverifiedTestnet) sendTopicRegister(to *Node, topics []Topic, idx int, pong []byte) {
	panic("sendTopicRegister called")
}

func (*preverifiedTestnet) Close() {}

func (*preverifiedTestnet) localAddr() *net.UDPAddr {
	return &net.UDPAddr{IP: net.ParseIP("10.0.1.1"), Port: 40000}
}

// verify generates a testnet struct literal with nodes at
// various distances to the given target.
func (n *preverifiedTestnet) verify(target NodeID) {
	n.target = target
	...
	fmt.Println("&preverifiedTestnet{")
	fmt.Printf("	target: %#v,\n", n.target)
