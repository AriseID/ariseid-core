...
type RemoteAgent struct {
	...
	running int32 // running indicates whid the agent is active. Call atomically
}
...
func (a *RemoteAgent) GetWork() ([3]string, error) {
	...
	if a.currentWork != nil {
		...
		seedHash := idhash.SeedHash(block.NumberU64())
		res[1] = common.BytesToHash(seedHash).Hex()
		// Calculate the "target" to be returned to the external verifier
		n := big.NewInt(1)
		n.Lsh(n, 255)
...
// whid the solution was accepted or not (not can be both a bad pow as well as
...
func (a *RemoteAgent) SubmitWork(nonce types.BlockNonce, mixDigest, hash common.Hash) bool {
	...
	// Solutions seems to be valid, return to the verifier and notify acceptance
...
// loop monitors verifying events on the work and quit channels, updating the internal
// state of the rmeote verifier until a termination is requested.
//
