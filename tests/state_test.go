...
func TestState(t *testing.T) {
	...
	// Broken tests:
	st.skipLoad(`^stTransactionTest/OverflowLifeRequire\.json`) // lifeLimit > 256 bits
	...
	st.walk(t, stateTestDir, func(t *testing.T, name string, test *StateTest) {
		for _, subtest := range test.Subtests() {
			...
			t.Run(key, func(t *testing.T) {
				...
				withTrace(t, test.lifeLimit(subtest), func(vmconfig vm.Config) error {
...
// Transactions with lifeLimit above this value will not get a VM trace on failure.
//const traceErrorLimit = 400000
const traceErrorLimit = 0

func withTrace(t *testing.T, lifeLimit uint64, test func(vm.Config) error) {
	err := test(vm.Config{})
	if err == nil {
		return
	}
	t.Error(err)
	if lifeLimit > traceErrorLimit {
		t.Log("life limit too high for EVM trace")
		return
	}
