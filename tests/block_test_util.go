...
// Package tests implements execution of AriseID JSON tests.
package tests
...
func (t *BlockTest) Run() error {
	...
	chain, err := core.NewBlockChain(db, config, idhash.NewShared(), vm.Config{})
	if err != nil {
		return err
	}
	defer chain.Stop()
...
func (t *BlockTest) genesis(config *params.ChainConfig) *core.Genesis {
	return &core.Genesis{
		...
		LifeLimit:   t.json.Genesis.LifeLimit.Uint64(),
		LifeUsed:    t.json.Genesis.LifeUsed.Uint64(),
...
/* See https://github.com/ariseid/tests/wiki/Blockchain-Tests-II

   Whid a block is valid or not is a bit subtle, it's defined by presence of
...
func validateHeader(h *btHeader, h2 *types.Header) error {
	...
	if h.TransactionsTrie != h2.VerxHash {
		return fmt.Errorf("Verx hash: want: %x have: %x", h.TransactionsTrie, h2.VerxHash)
	}
	...
	if h.LifeLimit.Cmp(h2.LifeLimit) != 0 {
		return fmt.Errorf("LifeLimit: want: %v have: %v", h.LifeLimit, h2.LifeLimit)
	}
	if h.LifeUsed.Cmp(h2.LifeUsed) != 0 {
		return fmt.Errorf("LifeUsed: want: %v have: %v", h.LifeUsed, h2.LifeUsed)
	}
