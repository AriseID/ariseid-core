...
// Reference: https://github.com/ariseid/wiki/wiki/Idhash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see idhash_io_prepare
enum idhash_io_rc {
...
/**
 * Logs a critical error in important parts of idhash. Should mostly help
...
#define ETHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("IDHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for idhash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the idhash
 ...
 *                           https://github.com/ariseid/wiki/wiki/Idhash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum idhash_io_rc
 */
enum idhash_io_rc idhash_io_prepare(
	char const* dirname,
	idhash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* idhash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* idhash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool idhash_mkdir(char const* dirname);
 ...
 */
bool idhash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* idhash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/ariseid/wiki/wiki/Idhash-DAG
 *
 ...
 */
bool idhash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool idhash_io_mutable_name(
	uint32_t revision,
