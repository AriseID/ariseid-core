/*
  ...
  along with cpp-ariseid.	If not, see <http://www.gnu.org/licenses/>.
*/
/** @file internal.c
...
// SeqMemoHash(s, R, N)
bool static idhash_compute_cache_nodes(
	node* const nodes,
	uint64_t cache_size,
	idhash_h256_t const* seed
)
{
...
)
{
	...
	for (uint32_t n = 0; n != max_n; ++n) {
		...
		idhash_calculate_dag_item(&(full_nodes[n]), n, light);
	}
	return true;
}

static bool idhash_hash(
	idhash_return_value_t* ret,
...
)
{
	...
	// pack hash and nonce toidder into first 40 bytes of s_mix
	...
	for (unsigned i = 0; i != ETHASH_ACCESSES; ++i) {
		...
		for (unsigned n = 0; n != MIX_NODES; ++n) {
			...
			if (full_nodes) {
				...
				node tmp_node;
				idhash_calculate_dag_item(&tmp_node, index * MIX_NODES + n, light);
				dag_node = &tmp_node;
			}
...
)
{
	...
	idhash_h256_t return_hash;
	idhash_quick_hash(&return_hash, header_hash, nonce, mix_hash);
	return idhash_check_difficulty(&return_hash, boundary);
}

idhash_light_t idhash_light_new_internal(uint64_t cache_size, idhash_h256_t const* seed)
{
	struct idhash_light *ret;
	...
	if (!idhash_compute_cache_nodes(nodes, cache_size, seed)) {
...
{
	idhash_h256_t seedhash = idhash_get_seedhash(block_number);
	idhash_light_t ret;
	ret = idhash_light_new_internal(idhash_get_cachesize(block_number), &seedhash);
...
)
{
  	idhash_return_value_t ret;
	ret.success = true;
	if (!idhash_hash(&ret, NULL, light, full_size, header_hash, nonce)) {
		ret.success = false;
	}
	return ret;
}
...
{
	uint64_t full_size = idhash_get_datasize(light->block_number);
	return idhash_light_compute_internal(light, full_size, header_hash, nonce);
}

static bool idhash_mmap(struct idhash_full* ret, FILE* f)
{
	int fd;
	char* mmapped_data;
	errno = 0;
	ret->file = f;
	if ((fd = idhash_fileno(ret->file)) == -1) {
		return false;
	}
	mmapped_data= mmap(
		NULL,
...
)
{
	...
	switch (idhash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, false)) {
	case ETHASH_IO_FAIL:
		// idhash_io_prepare will do all ETHASH_CRITICAL() logging in fail case
		goto fail_free_full;
	case ETHASH_IO_MEMO_MATCH:
		if (!idhash_mmap(ret, f)) {
	...
	case ETHASH_IO_MEMO_SIZE_MISMATCH:
		...
		if (idhash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, true) != ETHASH_IO_MEMO_MISMATCH) {
	...
	if (!idhash_compute_full_data(ret->data, full_size, light, callback)) {
...
{
	char strbuf[256];
	if (!idhash_get_default_dirname(strbuf, 256)) {
		return NULL;
	}
	uint64_t full_size = idhash_get_datasize(light->block_number);
	idhash_h256_t seedhash = idhash_get_seedhash(light->block_number);
	return idhash_full_new_internal(strbuf, seedhash, full_size, light, callback);
}
