...
const (
	...
	URN_WANIdnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANIdnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Idnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Idnet
//
...
// WANIdnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANIdnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANIdnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANIdnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANIdnetLinkConfig1Clients() (clients []*WANIdnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANIdnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANIdnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANIdnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANIdnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANIdnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANIdnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANIdnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANIdnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANIdnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANIdnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANIdnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANIdnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANIdnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANIdnetLinkConfig1 {
	clients := make([]*WANIdnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANIdnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewIdnetLinkStatus: allowed values: Up, Down
func (client *WANIdnetLinkConfig1) GetIdnetLinkStatus() (NewIdnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewIdnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANIdnetLinkConfig_1, "GetIdnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewIdnetLinkStatus, err = soap.UnmarshalString(response.NewIdnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
