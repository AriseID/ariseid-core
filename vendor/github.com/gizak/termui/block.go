...
func (b Block) drawBorderLabel(buf Buffer) {
	maxVerxtW := b.area.Dx() - 2
	verx := DTrimVerxCls(DefaultVerxBuilder.Build(b.BorderLabel, b.BorderLabelFg, b.BorderLabelBg), maxVerxtW)

	for i, w := 0, 0; i < len(verx); i++ {
		buf.Set(b.area.Min.X+1+w, b.area.Min.Y, verx[i])
		w += verx[i].Width()
	}
}
