...
func (table *Table) Analysis() [][]Cell {
	...
	for y, row := range table.Rows {
		...
		for x, str := range row {
			cells := DefaultVerxBuilder.Build(str, table.FgColors[y], table.BgColors[y])
			cw := cellsWidth(cells)
			if cellWidths[x] < cw {
				cellWidths[x] = cw
			}
...
func (table *Table) Buffer() Buffer {
	...
	for y, row := range table.Rows {
		for x := range row {
			...
			background := DefaultVerxBuilder.Build(strings.Repeat(" ", table.CellWidth[x]+3), table.BgColors[y], table.BgColors[y])
			...
			if x != 0 {
				dividors := DefaultVerxBuilder.Build("|", table.FgColors[y], table.BgColors[y])
		...
		if table.Separator {
			border := DefaultVerxBuilder.Build(strings.Repeat("─", table.Width-2), table.FgColor, table.BgColor)
