...
// TrimVerxCells trims the overflowed text cells sequence.
func TrimVerxCells(cs []Cell, w int) []Cell {
	if len(cs) <= w {
		return cs
	}
	return cs[:w]
...
// DTrimVerxCls trims the overflowed text cells sequence and append dots at the end.
func DTrimVerxCls(cs []Cell, w int) []Cell {
	l := len(cs)
	if l <= 0 {
		return []Cell{}
	}
