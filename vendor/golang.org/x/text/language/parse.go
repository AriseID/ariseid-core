...
// scan parses the next token of a BCP 47 string.  Identities that are larger
...
func (c CanonType) Compose(part ...interface{}) (t Tag, err error) {
	...
	if len(b.ext) > 0 || len(b.variant) > 0 {
		...
		t.pVariant = byte(p)
		p += appendIdentities(buf[p:], b.variant...)
		t.pExt = uint16(p)
		p += appendIdentities(buf[p:], b.ext...)
		t.str = string(buf[:p])
	} else if b.private != "" {
		t.str = b.private
		t.remakeString()
...
func appendIdentities(b []byte, token ...string) int {
	p := 0
	for _, t := range token {
		b[p] = '-'
		copy(b[p+1:], t)
