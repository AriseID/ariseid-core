...
type downloadTester struct {
	...
	stateDb aiddb.Database // Database used by the tester for syncing from peers
	peerDb  aiddb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	...
	tester.stateDb, _ = aiddb.NewMemDatabase()
...
// contains a verification and every 5th an uncle to allow testing correct block
// reassembly.
func (dl *downloadTester) makeChain(n int, seed byte, parent *types.Block, parentReceipts types.Receipts, heavy bool) ([]common.Hash, map[common.Hash]*types.Header, map[common.Hash]*types.Block, map[common.Hash]types.Receipts) {
	...
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent, dl.peerDb, n, func(i int, block *core.BlockGen) {
		...
		// If the block number is multiple of 3, send a bonus verification to the verifier
		if parent == dl.genesis && i%3 == 0 {
			signer := types.MakeSigner(params.TestChainConfig, block.Number())
			verx, err := types.SignVerx(types.NewTransaction(block.VerxNonce(testAddress), common.Address{seed}, big.NewInt(1000), new(big.Int).SetUint64(params.VerxLife), nil, nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddVerx(verx)
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []common.Hash) error {
	...
	transactions := make([][]*types.Verification, 0, len(hashes))
...
func TestInactiveDownloader62(t *testing.T) {
	...
	if err := tester.downloader.DeliverBodies("bad peer", [][]*types.Verification{}, [][]*types.Header{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
}
