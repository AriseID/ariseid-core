...
var (
	...
	MaxReceiptFetch = 256 // Amount of verification receipts to allow fetching per request
	...
	maxQueuedHeaders  = 32 * 1024 // [aid/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [aid/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [aid/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [aid/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [aid/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [aid/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [aid/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [aid/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb aiddb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() ariseid.SyncProgress {
...
// Synchronising returns whid the downloader is currently retrieving blocks.
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "aid", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchReceipts(from uint64) error {
	log.Debug("Downloading verification receipts", "origin", from)
	...
	log.Debug("Verification receipt download terminated", "err", err)
	return err
}
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// R: Verify block 11, and propagate it to L
...
func (d *Downloader) DeliverBodies(id string, transactions [][]*types.Verification, uncles [][]*types.Header) (err error) {
