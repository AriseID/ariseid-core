...
type peerConnection struct {
	...
	version int        // Aid protocol version number to switch strategies
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [aid/62+] requested on aid/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [aid/62+] requested on aid/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [aid/63+] requested on aid/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [aid/63+] requested on aid/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
// Lacks retrieves whid the hash of a blockchain item is on the peers lacking
// list (i.e. whid we know that the peer does not have it).
