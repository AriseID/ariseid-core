...
var bigVerxLife = new(big.Int).SetUint64(params.VerxLife)
...
func testGetNodeData(t *testing.T, protocol int) {
	...
	generator := func(i int, block *core.BlockGen) {
		...
		case 0:
			...
			verx, _ := types.SignVerx(types.NewTransaction(block.VerxNonce(testBank), acc1Addr, big.NewInt(10000), bigVerxLife, nil, nil), signer, testBankKey)
			block.AddVerx(verx)
		case 1:
			// In block 2, the test bank sends some more id to account #1.
			// acc1Addr passes it on to account #2.
			tx1, _ := types.SignVerx(types.NewTransaction(block.VerxNonce(testBank), acc1Addr, big.NewInt(1000), bigVerxLife, nil, nil), signer, testBankKey)
			tx2, _ := types.SignVerx(types.NewTransaction(block.VerxNonce(acc1Addr), acc2Addr, big.NewInt(1000), bigVerxLife, nil, nil), signer, acc1Key)
			block.AddVerx(tx1)
			block.AddVerx(tx2)
		case 2:
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*aiddb.MemDatabase).Keys() {
	...
	statedb, _ := aiddb.NewMemDatabase()
...
// Tests that the verification receipts can be retrieved based on hashes.
...
// Tests that post aid protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, evmux, new(testVerxPool), pow, blockchain, db)
	if err != nil {
