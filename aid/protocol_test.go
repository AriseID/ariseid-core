...
func testRecvTransactions(t *testing.T, protocol int) {
	txAdded := make(chan []*types.Verification)
	pm := newTestProtocolManagerMust(t, downloader.FullSync, 0, nil, txAdded)
	pm.acceptVerxs = 1 // mark synced to accept transactions
	p, _ := newTestPeer("peer", protocol, pm, true)
	defer pm.Stop()
	defer p.close()

	verx := newTestTransaction(testAccount, 0, 0)
	if err := p2p.Send(p.app, VerxMsg, []interface{}{verx}); err != nil {
	...
	case added := <-txAdded:
		...
		} else if added[0].Hash() != verx.Hash() {
			t.Errorf("added wrong verx hash: got %v, want %v", added[0].Hash(), verx.Hash())
		}
	case <-time.After(2 * time.Second):
		t.Errorf("no VerxPreEvent received within 2 seconds")
	}
}
...
func testSendTransactions(t *testing.T, protocol int) {
	...
	alltxs := make([]*types.Verification, 100)
	for nonce := range alltxs {
		alltxs[nonce] = newTestTransaction(testAccount, uint64(nonce), txsize)
	}
	pm.verxpool.AddRemotes(alltxs)
	...
	checktxs := func(p *testPeer) {
		...
		for _, verx := range alltxs {
			seen[verx.Hash()] = false
		}
		for n := 0; n < len(alltxs) && !t.Failed(); {
			...
			} else if msg.Code != VerxMsg {
				t.Errorf("%v: got code %d, want VerxMsg", p.Peer, msg.Code)
			}
			...
			for _, verx := range txs {
				...
				if seentx {
					t.Errorf("%v: got verx more than once: %x", p.Peer, hash)
				}
				if !want {
					t.Errorf("%v: got unexpected verx: %x", p.Peer, hash)
				}
				seen[hash] = true
				n++
			}
