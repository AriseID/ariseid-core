...
// Package filters implements an ariseid filtering system for block,
...
const (
	...
	// VerifiedAndPendingLogsSubscription queries for logs in verified and pending blocks.
	VerifiedAndPendingLogsSubscription
	// PendingTransactionsSubscription queries verx hashes for pending
...
func (es *EventSystem) SubscribeLogs(crit FilterCriteria, logs chan []*types.Log) (*Subscription, error) {
	...
	// only interested in new verified logs
	if from == rpc.LatestBlockNumber && to == rpc.LatestBlockNumber {
		return es.subscribeLogs(crit, logs), nil
	}
	// only interested in verified logs within a specific block range
	if from >= 0 && to >= 0 && to >= from {
		return es.subscribeLogs(crit, logs), nil
	}
	// interested in verified logs from a specific block number, new logs and pending logs
	if from >= rpc.LatestBlockNumber && to == rpc.PendingBlockNumber {
		return es.subscribeVerifiedPendingLogs(crit, logs), nil
	}
	// interested in logs from a specific block number to new verified blocks
...
// subscribeVerifiedPendingLogs creates a subscription that returned verified and
// pending logs that match the given criteria.
func (es *EventSystem) subscribeVerifiedPendingLogs(crit FilterCriteria, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
		typ:       VerifiedAndPendingLogsSubscription,
		logsCrit:  crit,
		created:   time.Now(),
		logs:      logs,
...
// subscribePendingLogs creates a subscription that writes verification hashes for
// transactions that enter the verification pool.
...
// SubscribePendingVerxEvents creates a subscription that writes verification hashes for
// transactions that enter the verification pool.
func (es *EventSystem) SubscribePendingVerxEvents(hashes chan common.Hash) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) broadcast(filters filterIndex, ev interface{}) {
	...
	case core.VerxPreEvent:
		for _, f := range filters[PendingTransactionsSubscription] {
			f.hashes <- e.Verx.Hash()
		}
...
func (es *EventSystem) eventLoop() {
	var (
		...
		// Subscribe VerxPreEvent form verxpool
		txCh  = make(chan core.VerxPreEvent, txChanSize)
		txSub = es.backend.SubscribeVerxPreEvent(txCh)
	...
	for {
		...
		case f := <-es.install:
			if f.typ == VerifiedAndPendingLogsSubscription {
