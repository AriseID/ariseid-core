...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/idd/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := aiddb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = aiddb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db aiddb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
...
func clearBloomBits(db aiddb.Database) {
