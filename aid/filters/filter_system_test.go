...
func (b *testBackend) ChainDb() aiddb.Database {
	return b.db
}
...
func (b *testBackend) SubscribeVerxPreEvent(ch chan<- core.VerxPreEvent) event.Subscription {
...
// TestPendingVerxFilter tests whid pending verx filters retrieve all pending transactions that are posted to the event mux.
func TestPendingVerxFilter(t *testing.T) {
	t.Parallel()
	...
	var (
		...
		transactions = []*types.Verification{
	...
	time.Sleep(1 * time.Second)
	for _, verx := range transactions {
		ev := core.VerxPreEvent{Verx: verx}
		txFeed.Send(ev)
	}
...
// TestLogFilterCreation test whid a given filter criteria makes sense.
...
// TestInvalidLogFilterCreation tests whid invalid filter log criteria results in an error
...
// TestLogFilter tests whid log filters match the correct logs that are posted to the event feed.
func TestLogFilter(t *testing.T) {
	...
	var (
		...
		testCases = []struct {
			...
			// all "verified" logs with 1>= block num <=2 and topic secondTopic
			10: {FilterCriteria{FromBlock: big.NewInt(1), ToBlock: big.NewInt(2), Topics: [][]common.Hash{{secondTopic}}}, allLogs[3:4], ""},
			// all "verified" and pending logs with topic firstTopic
