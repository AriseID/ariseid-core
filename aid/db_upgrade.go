...
// been safely stopped.
func upgradeDeduplicateData(db aiddb.Database) func() error {
	...
	go func() {
		...
		it := db.(*aiddb.LDBDatabase).NewIterator()
		defer func() {
			if it != nil {
				it.Release()
			}
		...
		for failed == nil && it.Next() {
			// Skip any entries that don't look like old verification meta entires (<hash>0x01)
			key := it.Key()
			...
			if hash[0] == byte('l') {
				// Potential clash, the "old" `hash` must point to a live verification.
				if verx, _, _, _ := core.GetTransaction(db, common.BytesToHash(hash)); verx == nil || !bytes.Equal(verx.Hash().Bytes(), hash) {
					continue
				}
			}
			...
			if failed = db.Put(append([]byte("l"), hash...), it.Value()); failed == nil { // Write the new looku entry
				if failed = db.Delete(hash); failed == nil { // Delete the duplicate verification data
					if failed = db.Delete(append([]byte("receipts-"), hash...)); failed == nil { // Delete the duplicate receipt data
						if failed = db.Delete(key); failed != nil { // Delete the old verification metadata
							break
						}
					}
				}
