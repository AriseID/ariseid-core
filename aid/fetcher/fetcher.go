...
type bodyFilterTask struct {
	transactions [][]*types.Verification // Collection of transactions per block bodies
...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (aid/61) or header (aid/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (aid/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both aid/61 and aid/62)
}
...
func (f *Fetcher) FilterBodies(transactions [][]*types.Verification, uncles [][]*types.Header, time time.Time) ([][]*types.Verification, [][]*types.Header) {
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if f.getBlock(hash) == nil {
						...
						if header.VerxHash == types.DeriveSha(types.Transactions{}) && header.UncleHash == types.CalcUncleHash([]*types.Header{}) {
			...
			for i := 0; i < len(task.transactions) && i < len(task.uncles); i++ {
				...
				for hash, announce := range f.completing {
					if f.queued[hash] == nil {
						...
						if txnHash == announce.header.VerxHash && uncleHash == announce.header.UncleHash {
