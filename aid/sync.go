...
// txsyncLoop takes care of the initial verification sync for each new
...
func (pm *ProtocolManager) txsyncLoop() {
	var (
		...
		sending = false               // whid a send is active
	...
	for {
		...
		case err := <-done:
			...
			if err != nil {
				pack.p.Log().Debug("Verification send failed", "err", err)
...
func (pm *ProtocolManager) synchronise(peer *peer) {
	...
	atomic.StoreUint32(&pm.acceptVerxs, 1) // Mark initial sync done
