...
// Package idd contains the simplified mobile APIs to AriseID.
//
// The scope of this package is *not* to allow writing a custom AriseID client
// with pieces plucked from AriseID, rather to allow writing native dapps on
...
// from the AriseID repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original AriseID version.
