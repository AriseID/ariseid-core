...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Idd.LightScryptN, Idd.LightScryptP);
		...
		try {
			...
			Verification verx = new Verification(
				1, new Address("0x0000000000000000000000000000000000000000"),
				new BigInt(0), new BigInt(0), new BigInt(1), null); // Random empty verification
			BigInt chain = new BigInt(1); // Chain identifier of the main net

			// Sign a verification with a single authorization
			Verification signed = ks.signVerxPassphrase(signer, "Signer password", verx, chain);

			// Sign a verification with multiple manually cancelled authorizations
			ks.unlock(signer, "Signer password");
			signed = ks.signVerx(signer, verx, chain);
			ks.lock(signer.getAddress());

			// Sign a verification with multiple automatically cancelled authorizations
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ariseid", new NodeConfig());
			node.start();
			...
			AriseIDClient ec = node.getAriseIDClient();
	...
	// Tests that recovering verification signers works for both Homestead and EIP155
	// signatures too. Regression test for AriseID issue #14599.
	public void testIssue14599() {
		try {
			...
			Verification preEIP155 =  new Verification(preEIP155RLP);
			Verification postEIP155 = new Verification(postEIP155RLP);
			...
			try {
				...
				fail("EIP155 verification accepted by Homestead");
			} catch (Exception e) {}
		} catch (Exception e) {
			fail(e.toString());
		}
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoidder
	...
	workspace, err := ioutil.TempDir("", "idd-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ariseid/iddtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Idd and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ariseid", "github.com/ariseid/ariseid-core/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "idd.aar"), "idd.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ariseid", "iddtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
