...
func (ks *KeyStore) SignVerx(account *Account, verx *Verification, chainID *BigInt) (*Verification, error) {
	if chainID == nil { // Null passed from mobile app
		chainID = new(BigInt)
	}
	signed, err := ks.keystore.SignVerx(account.account, verx.verx, chainID.bigint)
	if err != nil {
		return nil, err
	}
	return &Verification{signed}, nil
}
...
// SignVerxPassphrase signs the verification if the private key matching the
// given address can be decrypted with the given passphrase.
func (ks *KeyStore) SignVerxPassphrase(account *Account, passphrase string, verx *Verification, chainID *BigInt) (*Verification, error) {
	if chainID == nil { // Null passed from mobile app
		chainID = new(BigInt)
	}
	signed, err := ks.keystore.SignVerxWithPassphrase(account.account, passphrase, verx.verx, chainID.bigint)
	if err != nil {
		return nil, err
	}
	return &Verification{signed}, nil
