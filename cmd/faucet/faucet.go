...
// faucet is a Id faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the AriseID protocol")
	statsFlag   = flag.String("aidstats", "", "Aidstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Ids to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for AriseID and the faucet")
)

var (
	id = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Ids", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // AriseID address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Verx       *types.Verification `json:"verx"`       // Verification funding the account
}

// faucet represents a crypto faucet backed by an AriseID light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // AriseID protocol stack
	client *aidclient.Client   // Client connection to the AriseID chain
	index  []byte              // Index page to serve up on the web

	keystore *keystore.KeyStore // Keystore containing the single signer
	account  accounts.Account   // Account funding user faucet requests
	nonce    uint64             // Current pending nonce of the faucet
	price    *big.Int           // Current life value to issue funds with
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the AriseID light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := aid.DefaultConfig
	...
	// Assemble the aidstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightAriseID
			ctx.Service(&serv)
			return aidstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := aidclient.NewClient(api)
...
// close terminates the AriseID connection and tears down the faucet.
...
// apiHandler handles requests for Id grants and verification statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for AriseID addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No AriseID address found to fund"})
			continue
		}
		...
		if timeout = f.timeouts[gist.Owner.Login]; time.Now().After(timeout) {
			// User wasn't funded recently, create the funding verification
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), id)
			amount = new(big.Int).Mul(amount, new(big.Int).Exp(big.NewInt(5), big.NewInt(int64(msg.Tier)), nil))
			amount = new(big.Int).Div(amount, new(big.Int).Exp(big.NewInt(2), big.NewInt(int64(msg.Tier)), nil))

			verx := types.NewTransaction(f.nonce+uint64(len(f.reqs)), address, amount, big.NewInt(21000), f.price, nil)
			signed, err := f.keystore.SignVerx(f.account, verx, f.config.ChainId)
			if err != nil {
				websocket.JSON.Send(conn, map[string]string{"error": err.Error()})
				f.lock.Unlock()
				continue
			}
			// Submit the verification and mark as funded if successful
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance, _ := f.client.BalanceAt(context.Background(), f.account.Address, nil)
			balance = new(big.Int).Div(balance, id)

			price, _ := f.client.SuggestLifePrice(context.Background())
			nonce, _ := f.client.NonceAt(context.Background(), f.account.Address, nil)

			f.lock.Lock()
			f.price, f.nonce = price, nonce
			for len(f.reqs) > 0 && f.reqs[0].Verx.Nonce() < f.nonce {
				f.reqs = f.reqs[1:]
			}
			f.lock.Unlock()
