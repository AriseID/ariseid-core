...
import (
	...
	"github.com/ariseid/ariseid-core/contracts/ens"
	...
	"github.com/ariseid/ariseid-core/swarm"
	bzzapi "github.com/ariseid/ariseid-core/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:  "swap-api",
		Usage: "URL of the AriseID API provider to use to settle SWAP payments",
	}
	...
	EnsAPIFlag = cli.StringFlag{
		Name:  "ens-api",
		Usage: "URL of the AriseID API provider to use for ENS record lookups",
		Value: node.DefaultIPCEndpoint("idd"),
	}
...
// This init function sets defaults so cmd/swarm can run alongside idd.
func init() {
...
var app = utils.NewApp(gitCommit, "AriseID Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The AriseID Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.ariseid/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.ariseid/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.ariseid/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.ariseid/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	// exit if the deprecated --aidapi flag is set
	if ctx.GlobalString(DeprecatedEthAPIFlag.Name) != "" {
		utils.Fatalf("--aidapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := aidclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if ensapi != "" {
			...
			ensClient = aidclient.NewClient(client)
