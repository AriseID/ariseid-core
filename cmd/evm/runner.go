...
import (
	...
	"github.com/ariseid/ariseid-core/cmd/evm/internal/compiler"
	...
	"github.com/ariseid/ariseid-core/core/vm/runtime"
...
func runCmd(ctx *cli.Context) error {
	...
	initialLife := ctx.GlobalUint64(LifeFlag.Name)
	runtimeConfig := runtime.Config{
		Origin:   sender,
		State:    statedb,
		LifeLimit: initialLife,
		LifePrice: utils.GlobalBig(ctx, PriceFlag.Name),
		Value:    utils.GlobalBig(ctx, ValueFlag.Name),
		EVMConfig: vm.Config{
			Tracer:             tracer,
			Debug:              ctx.GlobalBool(DebugFlag.Name) || ctx.GlobalBool(MachineFlag.Name),
			DisableLifeMetering: ctx.GlobalBool(DisableLifeMeteringFlag.Name),
		},
	}
	...
	tstart := time.Now()
	var leftOverLife uint64
	if ctx.GlobalBool(CreateFlag.Name) {
		input := append(code, common.Hex2Bytes(ctx.GlobalString(InputFlag.Name))...)
		ret, _, leftOverLife, err = runtime.Create(input, &runtimeConfig)
	} else {
		if len(code) > 0 {
			statedb.SetCode(receiver, code)
		}
		ret, leftOverLife, err = runtime.Call(receiver, common.Hex2Bytes(ctx.GlobalString(InputFlag.Name)), &runtimeConfig)
	}
...
`, execTime, mem.HeapObjects, mem.Alloc, mem.TotalAlloc, mem.NumGC, initialLife-leftOverLife)
	}
	if tracer != nil {
		tracer.CaptureEnd(ret, initialLife-leftOverLife, execTime, err)
	} else {
		fmt.Printf("0x%x\n", ret)
		if err != nil {
			fmt.Printf(" error: %v\n", err)
