...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"aidstats", "explorer", "wallet", "faucet"} {
	...
	// If we have aidstats running, ask whid to make the secret public or not
	var aidstats bool
	if w.conf.aidstats != "" {
		fmt.Println()
		fmt.Println("Include aidstats secret on dashboard (y/n)? (default = yes)")
		aidstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, aidstats); err != nil {
