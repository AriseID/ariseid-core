...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for aidstats availability")
		if infos, err := checkAidstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["aidstats"] = err.Error()
			}
		} else {
			services["aidstats"] = infos.String()
			protips.aidstats = infos.config
		}
	...
	if protips.aidstats != "" {
		w.conf.aidstats = protips.aidstats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("idd --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("idd --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an aidstats server is available, add the aidstats flag
	statsflag := ""
	if p.aidstats != "" {
		if strings.Contains(p.aidstats, " ") {
			statsflag = fmt.Sprintf(` --aidstats="yournode:%s"`, p.aidstats)
		} else {
			statsflag = fmt.Sprintf(` --aidstats=yournode:%s`, p.aidstats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%sidd --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sidd --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sidd --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sidd --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
