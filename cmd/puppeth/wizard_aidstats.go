...
// deployAidstats queries the user for various input on deploying an aidstats
// monitoring server, after which it executes it.
func (w *wizard) deployAidstats() {
	...
	infos, err := checkAidstats(client, w.network)
	if err != nil {
		infos = &aidstatsInfos{
			port:   80,
			host:   client.server,
	...
	fmt.Println()
	fmt.Printf("Which port should aidstats listen on? (default = %d)\n", infos.port)
	infos.port = w.readDefaultInt(infos.port)

	// Figure which virtual-host to deploy aidstats on
	if infos.host, err = w.ensureVirtualHost(client, infos.port, infos.host); err != nil {
		log.Error("Failed to decide on aidstats host", "err", err)
		return
	}
	// Port and proxy settings retrieved, figure out the secret and boot aidstats
	fmt.Println()
	if infos.secret == "" {
	...
	// Try to deploy the aidstats server on the host
	...
	if out, err := deployAidstats(client, w.network, infos.port, infos.secret, infos.host, trusted, infos.banned); err != nil {
		log.Error("Failed to deploy aidstats container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
