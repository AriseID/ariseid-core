...
type config struct {
	...
	aidstats  string        // Aidstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // AriseID services known to be running on servers
...
// converts it to an AriseID address. If an empty line is entered, the default
// value is returned.
