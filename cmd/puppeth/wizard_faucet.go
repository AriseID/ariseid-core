...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Ids to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.aidstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.aidstats = w.readString() + ":" + w.conf.aidstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.aidstats)
		infos.node.aidstats = w.readDefaultString(infos.node.aidstats) + ":" + w.conf.aidstats
	}
