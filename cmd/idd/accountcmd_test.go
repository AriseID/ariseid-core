...
func TestAccountListEmpty(t *testing.T) {
	idd := runIdd(t, "account", "list")
	idd.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	idd := runIdd(t, "account", "list", "--datadir", datadir)
	defer idd.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	idd := runIdd(t, "account", "new", "--lightkdf")
	defer idd.ExpectExit()
	idd.Expect(`
...
`)
	idd.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	idd := runIdd(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer idd.ExpectExit()
	idd.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(idd.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(idd.StderrText(), m) {
...
// https://github.com/ariseid/ariseid-core/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	idd.SetTemplateFunc("keypath", func(file string) string {
