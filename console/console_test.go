...
func newTester(t *testing.T, confOverride func(*aid.Config)) *tester {
	...
	// Create a networkless protocol stack and start an AriseID service within
	...
	ethConf := &aid.Config{
		...
		IDbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(ethConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return aid.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register AriseID protocol: %v", err)
	}
