...
import (
	...
	"github.com/ariseid/ariseid-core/internal/jsre"
	"github.com/ariseid/ariseid-core/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute AriseID requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var aid = web3.aid; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = aid.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines aid.getBalance(aid.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Idd instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Idd metadata
	fmt.Fprintf(c.printer, "Welcome to the Idd JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + aid.coinbase);
		console.log("at block: " + aid.blockNumber + " (" + new Date(1000 * aid.getBlock(aid.blockNumber).timestamp) + ")");
