...
import (
	...
	"github.com/ariseid/ariseid-core/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of idd is "idd". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Idd", keep that.
	if name == "idd" || name == "idd-testnet" {
		name = "Idd"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "idd" instances.
var isOldIddResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by idd 1.4 are used if they exist.
	if c.name() == "idd" && isOldIddResource[path] {
		oldpath := ""
		if c.Name == "idd" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "ariseid-core-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
