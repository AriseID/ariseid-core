...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (aiddb.Database, error) {
	if ctx.config.DataDir == "" {
		return aiddb.NewMemDatabase()
	}
	db, err := aiddb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
