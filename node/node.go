...
func (n *Node) OpenDatabase(name string, cache, handles int) (aiddb.Database, error) {
	if n.config.DataDir == "" {
		return aiddb.NewMemDatabase()
	}
	return aiddb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
