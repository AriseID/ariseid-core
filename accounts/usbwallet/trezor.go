...
import (
	...
	"github.com/ariseid/ariseid-core/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whid the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whid the Trezor is opened, closed
// or whid the AriseID app was not started on it.
...
// SignVerx implements usbwallet.driver, sending the verification to the Trezor and
// waiting for the user to confirm or deny the verification.
func (w *trezorDriver) SignVerx(path accounts.DerivationPath, verx *types.Verification, chainID *big.Int) (common.Address, *types.Verification, error) {
	if w.device == nil {
		return common.Address{}, nil, accounts.ErrWalletClosed
	}
	return w.trezorSign(path, verx, chainID)
}

// trezorDerive sends a derivation request to the Trezor device and returns the
// AriseID address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AriseIDAddress)
	if _, err := w.trezorExchange(&trezor.AriseIDGetAddress{AddressN: derivationPath}, address); err != nil {
		return common.Address{}, err
	}
	return common.BytesToAddress(address.GetAddress()), nil
}

// trezorSign sends the verification to the Trezor wallet, and waits for the user
// to confirm or deny the verification.
func (w *trezorDriver) trezorSign(derivationPath []uint32, verx *types.Verification, chainID *big.Int) (common.Address, *types.Verification, error) {
	// Create the verification initiation message
	data := verx.Data()
	length := uint32(len(data))

	request := &trezor.AriseIDSignVerx{
		AddressN:   derivationPath,
		Nonce:      new(big.Int).SetUint64(verx.Nonce()).Bytes(),
		LifePrice:   verx.LifePrice().Bytes(),
		LifeLimit:   verx.Life().Bytes(),
		Value:      verx.Value().Bytes(),
		DataLength: &length,
	}
	...
	if chainID != nil { // EIP-155 verification, set chain ID explicitly (only 32 bit is supported!?)
		id := uint32(chainID.Int64())
		request.ChainId = &id
	}
	// Send the initiation message and stream content until a signature is returned
	response := new(trezor.AriseIDVerxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AriseIDVerxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
	...
	// Inject the final signature into the verification and sanity check the sender
