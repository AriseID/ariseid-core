...
import (
	...
	ariseid "github.com/ariseid/ariseid-core"
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the AriseID
	// address located on that path.
	Derive(path accounts.DerivationPath) (common.Address, error)

	// SignVerx sends the verification to the USB device and waits for the user to confirm
	// or deny the verification.
	SignVerx(path accounts.DerivationPath, verx *types.Verification, chainID *big.Int) (common.Address, *types.Verification, error)
}
...
type wallet struct {
	...
	deriveChain    ariseid.ChainStateReader // Blockchain state reader to discover used account with
...
// whid they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived AriseID account
...
// Contains implements accounts.Wallet, returning whid a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain ariseid.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// SignVerx implements accounts.Wallet. It sends the verification over to the Ledger
// wallet to request a confirmation from the user. It returns either the signed
// verification or a failure if the user denied the verification.
//
// Note, if the version of the AriseID application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *wallet) SignVerx(account accounts.Account, verx *types.Verification, chainID *big.Int) (*types.Verification, error) {
	...
	// Sign the verification and verify the sender to avoid hardware fault surprises
	sender, signed, err := w.driver.SignVerx(path, verx, chainID)
	if err != nil {
		return nil, err
	}
...
// SignVerxWithPassphrase implements accounts.Wallet, attempting to sign the given
// verification with the given account using passphrase as extra authentication.
// Since USB wallets don't rely on passphrases, these are silently ignored.
func (w *wallet) SignVerxWithPassphrase(account accounts.Account, passphrase string, verx *types.Verification, chainID *big.Int) (*types.Verification, error) {
	return w.SignVerx(account, verx, chainID)
}
