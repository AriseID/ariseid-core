...
// Callable method given a `Name` and whid the method is a constant.
// If the method is `Const` no verification needs to be created for this
// particular Method call. It can easily be simulated using a local VM.
// For example a `Balance()` method only needs to retrieve something
// from the storage and therefor requires no Verx to be send to the
// network. A method such as `Transact` does require a Verx and thus will
// be flagged `true`.
